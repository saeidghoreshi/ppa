...
<link rel="stylesheet" href="{$config.base_url}css/merchant-receipts.css" type="text/css" />
<!--<script src="{$config.base_url}js/receipt.js" type="text/javascript"></script>-->
        ...
        <table id="merchant_table" class="resizable">
            <tr>
            	<th class="amount{if $column == $smarty.const.TRANSACTION_ORDERBY_AMOUNT} sort{/if}">
                    <a href="{$site_url}/transaction/recent/{$smarty.const.TRANSACTION_ORDERBY_AMOUNT}/100/{$descend}">
                ...
                <th class="date{if $column == $smarty.const.TRANSACTION_ORDERBY_DATE} sort{/if}">
                    <a href="{$site_url}/transaction/recent/{$smarty.const.TRANSACTION_ORDERBY_DATE}/100/{$descend}">
                    Date{if $column == $smarty.const.TRANSACTION_ORDERBY_DATE}<img src="images/arrow_sort.png" alt="" />{/if}
                    </a>
                </th>
                <th class="time{if $column == $smarty.const.TRANSACTION_ORDERBY_TIME} sort{/if}">
                    <a href="{$site_url}/transaction/recent/{$smarty.const.TRANSACTION_ORDERBY_TIME}/100/{$descend}">
                    Time{if $column == $smarty.const.TRANSACTION_ORDERBY_TIME}
                    <img src="images/arrow_sort.png" alt="" />
                    {/if}
                    </a>
                </th>
                <th class="merch{if $column == $smarty.const.TRANSACTION_ORDERBY_USER} sort{/if}">
                    <a href="{$site_url}/transaction/recent/{$smarty.const.TRANSACTION_ORDERBY_USER}/100/{$descend}">
                    User{if $column == $smarty.const.TRANSACTION_ORDERBY_USER}
                    <img src="images/arrow_sort.png" alt="" />
                    {/if}
                    </a>
                </th>
                <th class="trans{if $column == $smarty.const.TRANSACTION_ORDERBY_ID} sort{/if}">
                    <a href="{$site_url}/transaction/recent/{$smarty.const.TRANSACTION_ORDERBY_ID}/100/{$descend}">
                ...
                <th class="status{if $column == $smarty.const.TRANSACTION_ORDERBY_STATUS} sort{/if}">
                    <a href="{$site_url}/transaction/recent/{$smarty.const.TRANSACTION_ORDERBY_STATUS}/100/{$descend}">
                ...
                <th class="flag{if $column == $smarty.const.TRANSACTION_ORDERBY_FLAG} sort{/if}">
                    <a href="{$site_url}/transaction/recent/{$smarty.const.TRANSACTION_ORDERBY_FLAG}/100/{$descend}">
            ...
            <tr id="trans-show-{$smarty.foreach.transaction.iteration}">
                ...
                <td class="bold">
                    <span class="data-4">User #{$t.tr_user_id}</span>
                </td>
                ...
                <td>
                    ...
                    {/if}
                    <input type="hidden" name="flag" value="{$t.transaction_flagged|escape}">
                    <input type="hidden" name="transId" value="{$t.transaction_id|escape}">
                    <input type="hidden" name="account_name" class="data-8" value="{$t.account_name|default:''|escape}">
                    <input type="hidden" name="merchant_address" class="data-9" value="{$t.merchant_street|default:'n/a'}, {$t.merchant_city|default:'n/a'}">
                    <input type="hidden" name="transaction_account" class="data-10" value="{$t.account_number|default:'n/a'|escape}">
                    <input type="hidden" name="transaction_paid" class="data-11" value="{$t.transaction_paid|default:'n/a'|escape}">
                    	...
                    	<div class='receiptDetail' style="background: #ffffff; padding: 2% 2% 0; font-size: 1; font-weight: normal; color: #666; width: 250px; border: 1px solid gray; overflow: auto;">       
					        <div id="receiptText" class="receiptText">
								<header style="text-align: center; margin-bottom: 25px; font-size: 220%;">
									<span class="receipt-vendor" style="font-size: 120%; font-family: 'courier new', courier, monospace;">{$t.merchant_name}</span>
									<address>
					        ...
					        <dl class="receipt-items" style="margin-bottom: 15px; overflow: hidden;">
								...
								<div class="clear"></div>
								<a href="{$config.base_url}merchant/transaction/recent#trans-receipts-row" id="print-link" class="hide-for-print" onclick="window.print();return false;" style="color: white; text-decoration: none; background-color: #666666; padding: 8px; -moz-border-radius: 10px; -webkit-border-radius: 10px; border-radius: 10px; font-family: 'courier new', courier, monospace; font-weight: bold;">Click to print receipt</a>
							</div>
							<div class="clear"></div>
							<dl>
