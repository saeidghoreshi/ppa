...
class User extends Ppa_controller
{
    ...
    {
        $user = $this->user_model->get_user_by_phone(3453454);
		echo var_dump($user);
		exit;
	}
    /**
    ...
    public function login()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                redirect($this->config->site_url(), 'redirect');
            }
        }
    }
    ...
    public function create()
    {
        ...
        if ($is_ajax)
        {
			...
			if( !$this->phone_check_on_create($_POST[FORM_PHONE]) )
			{
                $this->handle_ajax_request(AJAX_HTTP_401, array('status' => 'Phone number exists, please Log In') );
				return;
			}
			
			if( !$this->email_check_on_update($_POST[FORM_EMAIL]) )
			{
                $this->handle_ajax_request(AJAX_HTTP_401, array('status' => "Email already exists, please Log In.") );
				return;
			}
            ...
            if ($this->form_validation->run() == FALSE)
            {
                $this->handle_ajax_request(AJAX_HTTP_500,
                    array("status" => $this->form_validation->error_string('"', '"')) );
            }
            ...
            else
            {
                ...
                $account_array[ACCOUNT_NAME] = 'Gift Card';
                $account_array[ACCOUNT_BALANCE] = '0.00';
                $account_array[ACCOUNT_SECURITY_NUMBER] = md5('123');
                $account_array[ACCOUNT_SECURITY_PIN] = '';
                ...
                if (!empty($response->ResponseXml->RestException->Message))
                {
					...
					$response_obj = array( 'status' => current($response->ResponseXml->RestException->Message) );
                    $this->handle_ajax_request(AJAX_HTTP_500, $response_obj);
                }
			...
			//echo var_dump($_POST);
                ...
                if (!$this->email->send())
                {
                    $response_msg =
                        ...
                        '"}';
                    $this->handle_ajax_request(AJAX_HTTP_500, array("status" => $this->email->print_debugger()) );
                }
				else
				{
					$this->handle_ajax_request(AJAX_HTTP_200);
    ...
    public function new_passcode()
    {
        ...
        if ($is_ajax)
        {
            ...
            {
                $this->handle_ajax_request(AJAX_HTTP_500,
                    array("status" => "Passcode and Confirm Passcode must match"));
            }
