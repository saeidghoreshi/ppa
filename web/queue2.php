...
if (!empty($_GET['transaction_id'])) {
	if (empty($_GET['phone'])) {
		...
		$recepient_id = check_phone($_SESSION['user_phone']);
	} else {
		$action = 'PSS';
		$request_id = $_GET['transaction_id'];
		$recepient_id = check_phone($_GET['phone']);
	}
	...
	if (in_array($action, array('PR', 'PSS', 'PCS'))) {
		...
		$recepient_id = check_phone($_POST['phone']);
		$sender_id = intval($_SESSION['user_id']);
		...
		json_response($in, $status_code = '512', $status_msg = 'Incomlete Request');
		exit;
	}
}
...
$user_phone = check_phone($_POST['phone']);
...
if (isset($_POST['tips']))
	$in['tips'] = round(floatval($_POST['tips']), 2);
else
	$in['tips'] = round(floatval($in['tips']) / 10, 2);
if (!isset($_POST['tips']))
	$in['tips'] = round(floatval($in['tips']) / 10, 2);
...
if (isset($_POST['latitude']))
	$in['latitude'] = $_POST['latitude'];
if (isset($_POST['longitude']))
	$in['longitude'] = $_POST['longitude'];
if (isset($_POST['geots']))
	$in['geots'] = $_POST['geots'];

// merchant GEO
if (isset($_SESSION['latitude']))
	$in['latitude'] = $_SESSION['latitude'];
if (isset($_SESSION['longitude']))
	$in['longitude'] = $_SESSION['longitude'];
...
if (empty($_SESSION) || empty($_SESSION['user_id'])) {
	json_response($in, $status_code = '401', $status_msg = 'Unauthorized');
	exit;
}
...
//echo $in['transaction_id'].'_';
			...
			$in['merchant_accuracy'] = $_SESSION['merchant_accuracy'];
...
//echo var_dump($in);
			if (!$response['status']) {
				json_response($response, $status_code = '501', $status_msg = $response['msg']);
				exit;
			}
			else
				json_response($in);
		...
		case 'PSR':
			...
			if (!$response['status']) {
				json_response($response, $status_code = '502', $status_msg = $response['msg']);
				exit;
			}
			$in = array_merge($in, $response);
			...
			if (!$response['status']) {
				json_response($response, $status_code = '503', $status_msg = $response['msg']);
				exit;
			}
			else
				json_response($in);
		...
		case 'PLR':
			...
			if (!$response['status']) {
				json_response($response, $status_code = '525', $status_msg = $response['msg']);
				exit;
			}
			$in = array_merge($in, $response);
			json_response($in);
		...
		case 'PSS':
			...
			//var_dump($in);
			if (!$in['status'])
				json_response($in, $status_code = '200', $status_msg = $in['msg']);
			else
				json_response($in);
		...
		case 'PPR':
			...
			$response = payment_status($in);
			if (!$response['status']) {
				json_response($response, $status_code = '505', $status_msg = $response['msg'] . '1');
				exit;
			}
			$in = array_merge($in, $response);
			if (isset($_POST['tips']))
			...
			if (isset($_POST['account_type']))
				$in['account_type'] = intval($_POST['account_type']);
			if (isset($_POST['recepient_latitude']))
				$in['recepient_latitude'] = $_POST['recepient_latitude'];
			if (isset($_POST['recepient_longitude']))
				$in['recepient_longitude'] = $_POST['recepient_longitude'];
			$in['account_id'] = $_POST['account_id'];
			if ($in['sender_cancelled'] && $in['recepient_cancelled']) {
				json_response($response, $status_code = '506', 'Transaction has been already cancelled');
				exit;
			}
			$in['recepient_received'] = true;
			$in['sender_notified'] = false;
			if ($in['recepient_paid']) {
				json_response($response, $status_code = '507', $status_msg = 'Transaction has been already approved');
				exit;
			}
			if (empty($in['account_id'])) {
				json_response($in, $status_code = '200', $status_msg = 'Select Payment option');
				exit;
			}
			
			// set default accuracy to 300m
			$merchant_accuracy = empty( $in['merchant_accuracy'] ) ? 300 : intval($in['merchant_accuracy']);
			
			if (intval($in['distance']) > $merchant_accuracy) {
				json_response($in, $status_code = '508', $status_msg = 'Merchant is too far from you - ' . $in['distance'] . 'm');
				exit;
			}
			if (isset($_POST['pin']))
				$in['pin'] = $_POST['pin'];
			if (!payment_persist($in, $mode = 'pin_check')) {
				json_response($in, $status_code = '200', $status_msg = 'PIN incorrect');
				exit;
			}
			...
			else if (!empty($in['account_type']) && $in['account_type'] == 9) {
				// Paypal
				$ret = paypal_push($in);
				if( isset($ret['transaction_merchant_note']) ) $in['transaction_merchant_note'] = $ret['transaction_merchant_note'];
				//$ret['status'] = 'OK';
			}
			else {
				// Regular CC Payment
			...
			//echo print_r($ret);
			if ($ret['status'] != 'OK') {
				json_response($response, $status_code = '500', $status_msg = $ret['msg']);
				exit;
			}
			// todo: tips
			if( !isset($in['transaction_merchant_note']) ) $in['transaction_merchant_note'] = '';
			if (!payment_persist($in, $mode = 'pay')) {
				json_response($response, $status_code = '515', $status_msg = 'Payment was not saved to database');
				exit;
			}
			...
			$response = payment_update($in, $mode = 'recepient');
			if (!$response['status']) {
				json_response($response, $status_code = '508', $status_msg = $response['msg']);
				exit;
			}
			else
				json_response($in);
		...
		case 'PCR':
			...
			$response = payment_status($in);
			if (!$response['status']) {
				json_response($response, $status_code = '509', $status_msg = $response['msg']);
				exit;
			}
			$in = array_merge($in, $response);
			if ($in['recepient_paid'])
				json_response($response, $status_code = '510', $status_msg = 'Transaction has been already approved');
			$in['recepient_received'] = true;
			$in['sender_notified'] = false;
			...
			if (isset($_POST['recepient_longitude']))
				$in['recepient_longitude'] = $_POST['recepient_longitude'];
			if (!payment_persist($in, $mode = 'cancel')) {
				json_response($response, $status_code = '515', $status_msg = 'Payment was not saved to database');
				exit;
			}
			$in['db_saved'] = true;
			$response = payment_update($in, $mode = 'recepient');
			if (!$response['status']) {
				json_response($response, $status_code = '511', $status_msg = $response['msg']);
				exit;
			}
			else
				json_response($in);
		...
		case 'PCS':
			...
			$response = payment_status($in);
			if (!$response['status']) {
				json_response($response, $status_code = '512', $status_msg = $response['msg']);
				exit;
			}
			$in = array_merge($in, $response);
			if ($in['recepient_paid']) {
				json_response($response, $status_code = '513', $status_msg = 'Transaction has been already approved');
				exit;
			}
			...
			if (!$response['status']) {
				json_response($response, $status_code = '514', $status_msg = $response['msg']);
				exit;
			}
			else
				json_response($in);
...
function paypal_push($in, $mode='') {
	...
	$in = array_merge($in, paypal_info($in));

	if (empty($in['trnOrderNumber']) || empty($in['trnAmount']) || empty($in['customerCode']) 
			|| empty($in['paypal_preapproval']) || empty($in['paypal_sender']) || empty($in['paypal_receiver']) ) {
		return array('status' => 'Error', 'msg' => 'Make sure all payment transaction parameters(trnOrderNumber, trnAmount, customerCode, paypal_preapproval, paypal_sender, paypal_receiver) provided');
	}
	...
	try {
		$returnURL = "http://www.payphoneapp.com/index.php/transaction";
		$cancelURL = "http://www.payphoneapp.com/index.php/account/paypal_cancel";
		$currencyCode = 'CAD';
		$email = $in['paypal_sender'];
		$preapprovalKey = $in['paypal_preapproval'];
		$receiverEmail = $in['paypal_receiver'];
		$amount = $in['trnAmount'];
		$applicationId = 'APP-4D611114R9015663L';
		...
		$payRequest->clientDetails->applicationId = $applicationId;
		$payRequest->clientDetails->deviceId = '';
		...
		if (strtoupper($ap->isSuccess) == 'FAILURE') {
			...
			if ($response->paymentExecStatus == "COMPLETED" && 
				...
				return array('status' => 'OK', 'transaction_merchant_note' => $response->payKey);
			} else {
...
function paypal_info($in, $mode='') {
	...
	$sql = "SELECT a.account_number AS paypal_preapproval, a.account_email AS paypal_sender, u.paypal_email AS paypal_receiver 
			FROM account a 
			INNER JOIN user u ON u.user_id = '" . intval($in['sender_id']) . "'
			WHERE a.account_id = '" . intval($in['account_id']) . "'";
	//echo $sql;
...
function payment_persist($in, $mode='') {
	switch ($mode) {
		case 'pin_check':
			// TODO: add user check
			$sql = "SELECT account_id FROM account WHERE account_id = '" . intval($in['account_id']) . "' AND account_enabled = 1 AND account_security_pin IN ('','" . mysql_real_escape_string($in['pin']) . "') LIMIT 1";
			//echo $sql;
			break;
		case 'gc_update':
...
AND accounttype.merchant_id IN (0,'" . intval($in['merchant_id']) . "') AND account.account_balance >= '" . (floatval($in['amount']) + floatval($in['tips'])) . "'";
//echo $sql;
			}
			break;
		case 'pay':
...
'" . mysql_real_escape_string($in['transaction_merchant_note']) . "' AS transaction_merchant_note, '' AS transaction_user_note, '1' AS transaction_paid, '0' AS transaction_cancelled, '0' AS transaction_flagged,
...
function check_phone($phone) {
		...
		$sql = "SELECT user_phone FROM v2.user WHERE user_phone LIKE '%".intval($phone)."' AND user_enabled = 1;";
		$result = mysql_query($sql);
		if (!$result) {
			echo('Could not query:' . mysql_error());
			mysql_close($link);
		...
		if (mysql_num_rows($result) == 1 && $row = mysql_fetch_row($result) ) {
			mysql_close($link);
			return $row[0];
		}
...
function payment_update($in, $mode='unknown') {
	...
	switch ($mode) {
		case 'sender':
			if (isset($in['sender_notified']))
				$msg[$transaction_id]['sender_notified'] = $in['sender_notified'];
			if (isset($in['sender_cancelled']))
				$msg[$transaction_id]['sender_cancelled'] = $in['sender_cancelled'];
			if (isset($in['recepient_received']))
				$msg[$transaction_id]['recepient_received'] = $in['recepient_received'];
			break;
		case 'recepient':
			if (isset($in['recepient_received']))
				$msg[$transaction_id]['recepient_received'] = $in['recepient_received'];
			if (isset($in['recepient_paid']))
				$msg[$transaction_id]['recepient_paid'] = $in['recepient_paid'];
			if (isset($in['recepient_cancelled']))
				$msg[$transaction_id]['recepient_cancelled'] = $in['recepient_cancelled'];
			if (isset($in['tips']))
				$msg[$transaction_id]['tips'] = $in['tips'];
			if (isset($in['sender_notified']))
				$msg[$transaction_id]['sender_notified'] = $in['sender_notified'];
			if (isset($in['latitude']))
				$msg[$transaction_id]['recepient_latitude'] = $in['latitude'];
			if (isset($in['longitude']))
				$msg[$transaction_id]['recepient_longitude'] = $in['longitude'];
			if (isset($in['geots']))
				$msg[$transaction_id]['recepient_geots'] = $in['geots'];
...
function payment_request($in) {
	...
	$merchant_accuracy = intval($in['merchant_accuracy']);
	...
	$msg[$transaction_id] = array('transaction_id' => $transaction_id, 'sender_id' => $sender_id, 'recepient_received' => false, 'sender_notified' => true,
		...
		'parameters' => $parameters, 'transaction_datetime_requested' => date("Y-m-d H:i:s"), 'merchant_accuracy' => $merchant_accuracy);
	if (isset($in['latitude']))
		$msg[$transaction_id]['sender_latitude'] = $in['latitude'];
	if (isset($in['longitude']))
		$msg[$transaction_id]['sender_longitude'] = $in['longitude'];
	if (!store($key = $recepient_id, $data = $msg, $ttl = 300)) {
		// try saving again
		if (!store($key = $recepient_id, $data = $msg, $ttl = 300)) {
			return array('status' => false, 'msg' => 'Payment Queue Message Saving Failed');
